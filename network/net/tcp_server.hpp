    ...
    namespace net
    {
        ...
        public:
            ...
            tcp_server(const std::string& ip, uint16_t port, const server_options& options, irouter_ptr router_ptr = nullptr)
                ...
                , options_(options)
                , cur_conn_id_(0)
                , log_(cytx::log::force_get_log("net"))
            {
                log_->debug("create server tcp://{}:{}, thread_mode {}, io thread count {}", ip, port, (int32_t)options.thread_mode, ios_pool_.size());
            }

            void start()
            {
                log_->debug("tcp server start");
                do_accept();
                ios_pool_.start();
            }
            ...
            void stop()
            {
                log_->debug("tcp server begin stop");
                ios_pool_.stop();
            }
            ...
            connection_ptr create_connection()
            {
                ...
                log_->debug("tcp server create connection {}, ios index {}, batch_send_msg {}, disconnect_interval {}", cur_conn_id_,
        ...
        private:
            void do_accept()
            {
                ...
                log_->debug("tcp server do accept");
                    ...
                    [this, new_connection](const ec_t& err) mutable
                {
                    if (!err)
                    {
                        log_->debug("tcp server accepted on connection {}", new_connection->get_conn_id());
                        if (router_ptr_)
                    ...
                    {
                        log_->error("accept error, {}", err.message());
                    }
